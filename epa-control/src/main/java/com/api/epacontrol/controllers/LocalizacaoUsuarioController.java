...
public class LocalizacaoUsuarioController {
    ...
    @PostMapping
    public ResponseEntity<Object> saveLocalizacaoUsuario(@RequestBody @Valid LocalizacaoUsuarioDto localizacaoUsuarioDto) {
        if (localizacaoUsuarioService.existsById(localizacaoUsuarioDto.getId())) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body("Localização já existe!");
        }

        var localizacaoUsuario = new LocalizacaoUsuario();
        BeanUtils.copyProperties(localizacaoUsuarioDto, localizacaoUsuario);
        return ResponseEntity.status(HttpStatus.CREATED).body(localizacaoUsuarioService.save(localizacaoUsuario));
    }
