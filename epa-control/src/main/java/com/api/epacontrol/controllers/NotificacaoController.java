...
public class NotificacaoController {
  ...
  @PostMapping
  public ResponseEntity<Object> saveNotificacao(
    @RequestBody @Valid NotificacaoDto notificacaoDto
  ) {
    if (notificacaoService.existsById(notificacaoDto.getId())) {
      return ResponseEntity
        .status(HttpStatus.CONFLICT)
        .body("Notificação já existe!");
    }

    Optional<UsersModel> recipientOptional = usersService.findById(
      notificacaoDto.getRecipientId()
    );
    ...
    return ResponseEntity
      ...
      .body(notificacaoService.save(notificacao));
  }
}
